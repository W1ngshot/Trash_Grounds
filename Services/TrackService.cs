    ...
    */
    ...
    private static IEnumerable<TrackCardModel> CreateCardModels(List<Track> tracks)
    {
        ...
        foreach (var track in tracks)
        {
            ...
            var item = new TrackCardModel { TrackId = track.Id, TrackName = track.Title, AuthorName = track.User.Nickname, AuthorId = track.User.Id, Rating = rate, PreviewUrl = $@"/assets/img/{track.Id}.jpg", ListensCount = track.Listens_Count};
            result.Add(item);
        }
        return result;
    }
    ...
    public static IEnumerable<TrackCardModel> GetAnotherTracksFromUser(int trackId, User user)
    {
        using ApplicationContext db = new ApplicationContext();
        var tracks = db.Tracks
            .Include(t => t.User)
            .Where(t => t.User == user && t.Id != trackId)
            .ToList()
            .OrderByDescending(t => t.GetTrackRate())
            .Take(4)
            .ToList();
        return CreateCardModels(tracks);
    }

    public static IEnumerable<TrackCardModel> GetUserTracks(User user)
    {
        using ApplicationContext db = new ApplicationContext();
        var tracks = db.Tracks
            .Include(t => t.User)
